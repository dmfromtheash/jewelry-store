...
| Authenticated admin | `npm run smoke:admin` | 14 admin surfaces (incl. dashboard + inventory + readiness + seo + reviews + customers + promotions + email-outbox) render **with** a local session, **gated** without |
...
| External analytics / ad pixels / retargeting / paid ads / affiliate / marketing automation/CRM | 🚫 | — (not built) | **No** | no analytics/events, no Meta/Google/TikTok pixel, no retargeting/ads/affiliate, no marketing automation or CRM |
